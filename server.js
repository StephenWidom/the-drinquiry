...
io.on("connection", (socket) => {
        ...
        if (host) {
            ...
            if (!_.isNil(thisPlayer)) {
                if (thisPlayer.connected) {
                    ...
                    if (game.started) socket.emit("gameStarted", true);
                    return;
                }
            }
    ...
    socket.on("changeCharacter", () => {
        ...
        if (!_.isNil(thisPlayer)) {
            ...
            const reservedNames = ["plumson", "fuchiass", "obi"];
            if (reservedNames.includes(thisPlayer.character.class)) return console.log("nice name");
    ...
    socket.on("updateHealth", (id, modifier) => {
        ...
        if (!_.isNil(thisPlayer)) {
            ...
            if (thisPlayer.health > 5) thisPlayer.health = 5;
    ...
    socket.on("updatePotions", (id, modifier) => {
        ...
        if (!_.isNil(thisPlayer)) {
            ...
            if (thisPlayer.potions > 3) thisPlayer.potions = 3;

            if (thisPlayer.potions < 0) thisPlayer.potions = 0;
            io.emit("updatePlayers", players);
        }
    });
    ...
    socket.on("initMonster", (monster) => {
        ...
        if (game.challenge !== "roker")
            // Already updating prompt in fetchWeather
    ...
    socket.on("hitMonster", () => {
        ...
        if (game.triviaCategory || game.challenge === "roker") {
            ...
            if (game.challenge !== "roker") io.emit("updatePrompt", game.prompt, game.challenge, game.triviaCategory, game.city);
        }
    });
    ...
    socket.on("fuckedUp", (id) => {
        ...
        const thisPlayer = players.find((p) => p.id === id);
        if (!_.isNil(thisPlayer)) {
            if (!thisPlayer.shield || health > 1) {
                // Health must be 1 because one was subtracted by accidental hit
                takeDamage(thisPlayer);
    ...
    socket.on("revealTemperature", () => {
        const { weather } = game;
        if (_.isNil(weather)) return false;
    ...
    socket.on("shufflePlayers", (haunt = false) => {
        ...
        game.players = shuffledPlayers;
        io.emit("updatePlayers", game.players);
        if (haunt) io.to(game.host).emit("playGhostSound");
    });
...
const nextPlayer = (id, players) => {
    ...
    if (players.every((p) => p.dead || !p.connected)) return false;

    const playerIndex = players.findIndex((p) => p.id === id);
    if (playerIndex === -1) return false;

    let actualIndex = playerIndex + 1;
    if (actualIndex >= players.length) actualIndex = 0;

    if (players[actualIndex].dead || !players[actualIndex].connected) return nextPlayer(players[actualIndex].id, players);
...
const checkWinCondition = () => {
    ...
    if (alivePlayers.length === 0) {
        ...
        io.emit("gameWon", game.winner);
    } else if (alivePlayers.length === 2 && alivePlayers.filer((p) => p.health === 1).length === 1) {
...
const pickEvent = (player) => {
    ...
    if (randEvent.src === "scroll_yellow" && (player.scroll || game.suddenDeath)) return pickEvent(player);

    // Don't give em an amulet if they've got one
    if (randEvent.src === "urand_bloodlust_new" && player.amulet) return pickEvent(player);

    // Don't draw the discard item one if they don't have items
    if (randEvent.src === "unseen_item_old" && !player.amulet && !player.scroll) return pickEvent(player);

    // Don't let the poisonous cloud actually kill a player
    if (randEvent.src === "cloud_meph_2" && player.health === 1) return pickEvent(player);

    // Don't give everyone scrolls if they've all got one
    if (randEvent.src === "ru" && game.players.every((p) => p.scroll)) return pickEvent(player);

    // Don't fucks wit some events during sudden death
    if (game.suddenDeath && (randEvent.src === "necromutation_old" || randEvent.src === "misc_lantern" || randEvent.src === "rune_abyss" || randEvent.src === "cloud_black_smoke")) return pickEvent(player);

    // Don't revive dead players if there are none
    if (randEvent.src === "gem_brass_new" && !game.players.some((p) => p.dead)) return pickEvent(player);
...
const getPrompt = () => {
    if (game.challenge !== "trivia") game.triviaCategory = null;

    if (game.challenge !== "roker") game.city = null;
    ...
    switch (game.challenge) {
        ...
        case "trivia":
            ...
            if (questionObj.question.trim() === "") return getPrompt();
...
const fetchWeather = (city) => {
    if (!city || _.isNil(city.id)) return false;

    fetch(`https://api.openweathermap.org/data/2.5/weather?id=${city.id}&units=imperial&APPID=${apiKey}`)
        .then((response) => response.json())
        .then((response) => {
...
const endBattle = (updatePlayers = true) => {
    ...
    if (updatePlayers) io.emit("updatePlayers", players);
};
